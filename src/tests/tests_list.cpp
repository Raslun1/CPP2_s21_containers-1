...
TEST(List, Splice) {
  ...
  std::list<int>::const_iterator std_it = std_list_first.cbegin();
...
TEST(List, SortInt) {
  s21::list<int> l{1, 0, 6, 10, 111, 6, 5, 120, 0, 0, 2};
  s21::list<int> sorted{0, 0, 0, 1, 2, 5, 6, 6, 10, 111, 120};

  l.sort();

  s21::list<int>::iterator iter1 = l.begin();
  s21::list<int>::iterator iter2 = sorted.begin();

  for (; iter1 != l.end() && iter2 != sorted.end(); ++iter1, ++iter2) {
    EXPECT_EQ(*iter1, *iter2);
  }
}

TEST(List, SortChar) {
  s21::list<char> l{'m', 'n', 'a', 'z', 'b', 'v', 'c'};
  s21::list<char> sorted{'a', 'b', 'c', 'm', 'n', 'v', 'z'};

  l.sort();

  s21::list<char>::iterator iter1 = l.begin();
  s21::list<char>::iterator iter2 = sorted.begin();
...
TEST(List, SortString) {
  s21::list<std::string> l{"acb", "nmb", "abc"};
  s21::list<std::string> sorted{"abc", "acb", "nmb"};

  l.sort();

  s21::list<std::string>::iterator iter1 = l.begin();
  s21::list<std::string>::iterator iter2 = sorted.begin();
